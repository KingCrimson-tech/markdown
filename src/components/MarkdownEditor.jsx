...
const MarkdownEditor = () => {
  ...
  useEffect(() => {
    localStorage.setItem('markdown-editor-dark-mode', JSON.stringify(isDarkMode));
    if (isDarkMode) {
  ...
  const renderMarkdown = (text) => {
    return text
      ...
      .replace(/^\s*- \[x\] (.*$)/gim, '<div style="margin: 0.5em 0;"><input type="checkbox" checked disabled style="margin-right: 0.5em;">$1</div>')
      .replace(/^\s*- \[ \] (.*$)/gim, '<div style="margin: 0.5em 0;"><input type="checkbox" disabled style="margin-right: 0.5em;">$1</div>')
    ...
    <div className={`h-screen flex flex-col ${isDarkMode ? 'dark bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {(viewMode === 'write' || viewMode === 'split') && (
          <div className={`${viewMode === 'split' ? 'w-1/2' : 'w-full'} border-r border-gray-300 dark:border-gray-700 flex flex-col`}>
            <div className="p-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
              <span className="text-sm text-gray-600 dark:text-gray-400 font-mono">Editor</span>
            </div>
            <textarea
        ...
        {(viewMode === 'preview' || viewMode === 'split') && (
          <div className={`${viewMode === 'split' ? 'w-1/2' : 'w-full'} flex flex-col`}>
            <div className="p-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
              <span className="text-sm text-gray-600 dark:text-gray-400 font-mono">Preview</span>
            </div>
      ...
      <div className="border-t border-gray-300 dark:border-gray-700 p-2 text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 flex justify-between items-center">
        <div>
          <span className="mr-4">Words: {markdown.split(/\s+/).filter(word => word.length > 0).length}</span>
